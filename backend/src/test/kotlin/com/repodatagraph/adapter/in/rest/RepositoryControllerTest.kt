...
class RepositoryControllerTest {
    ...
    fun `POST repositories registers through the node use case and returns 201`() {
        ...
        mockMvc
            ...
            .andExpect(jsonPath("$.url").value("https://github.com/acme/payments"))
    ...
    /**
     * The remote is passed through as the caller wrote it, not canonicalised here.
     *
     * It used to be expanded in this controller, which meant two places decided what a remote means:
     * this one and the node use case the other endpoint goes through. Now there is one parser, run
     * once, inside the use case - so a shorthand posted here and the same shorthand posted to
     * /api/v1/nodes/Repository cannot land on different nodes (#8). That the result *is*
     * canonicalised is covered where it now happens, by NodeServiceTest and the acceptance suite.
     */
    @Test
    fun `POST repositories hands the remote to the use case as written`() {
        ...
        mockMvc
            .perform(
                post("/api/v1/repositories")
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(objectMapper.writeValueAsString(CreateRepositoryRequest(url = "Acme/Payments"))),
            ).andExpect(status().isCreated)
    ...
    fun `GET repositories returns list`() {
        whenever(repositoryUseCase.listRepositories()).thenReturn(
            listOf(Repository(id = "1", url = "https://github.com/org/repo", host = "github.com", org = "org", name = "repo")),
        )

        mockMvc
            .perform(get("/api/v1/repositories"))
            .andExpect(status().isOk)
            .andExpect(jsonPath("$[0].url").value("https://github.com/org/repo"))
    }

    @Test
    fun `GET repository by id returns 200`() {
        whenever(repositoryUseCase.getRepository("1")).thenReturn(
            Repository(id = "1", url = "https://github.com/org/repo", host = "github.com", org = "org", name = "repo"),
        )
