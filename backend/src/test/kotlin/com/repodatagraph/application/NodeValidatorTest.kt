 ...
 * than a rule per node type. A type added to the ontology is validated without touching this class.
 */
class NodeValidatorTest {
    ...
    fun `a missing required property is reported against its own field`() {
        val errors = validator.validate(everyType, mapOf("count" to 1))
    ...
    fun `a required property present but blank is still missing`() {
        val errors = validator.validate(everyType, mapOf("name" to "   "))
    ...
    fun `a property the ontology does not declare is refused`() {
        val errors = validator.validate(everyType, mapOf("name" to "sample", "colour" to "blue"))
    ...
    fun `identity and provenance cannot be supplied by the client`() {
        val errors = validator.validate(everyType, mapOf("name" to "sample", "id" to "Sample:x", "key" to "x"))
    ...
    fun `a value of the wrong type names the type the ontology declares`(
        ...
        val errors = validator.validate(everyType, mapOf("name" to "sample", field to value))
    ...
    fun `a string list with a non-string element is refused`() {
        val errors = validator.validate(everyType, mapOf("name" to "sample", "topics" to listOf("a", 1)))
    ...
    fun `an Instant is accepted as well as its string form`() {
        val errors = validator.validate(everyType, mapOf("name" to "sample", "seenAt" to Instant.EPOCH))
    ...
    fun `an optional property that is absent is not an error, and neither is an explicit null`() {
        val errors = validator.validate(everyType, mapOf("name" to "sample", "count" to null))
    ...
    fun `every problem is reported at once, so a form can show them all`() {
        val errors = validator.validate(everyType, mapOf("count" to "x", "colour" to "blue"))
