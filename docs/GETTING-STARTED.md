...
[user guide](USER-GUIDE.md) lists them.
...
[The testing guide](TESTING.md) describes each suite, what runs on commit and push, and why tests are
deliberately not run on commit.

## Where to go next

- The [user guide](USER-GUIDE.md) explains the web interface and the API in full.
- The [ontology guide](ONTOLOGY.md) explains the model: the node and relationship types, how identity is
  derived, and what provenance is recorded.
- The [roadmap](ROADMAP.md) says what is built and what is planned, issue by issue.
